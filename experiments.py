...
class Experiment(object):
    ...
    def run(self) -> Tuple[Tensor, Tensor]:
        ...
        for num_fold in folds:
            ...
            best_losses_n_metrics, edge_ratios = self.single_fold(
        ...
        # prints
        print(
            f"Final Rewired train={round(metrics_mean[0], self.decimal)},"
        ...
        if len(folds) > 1:
            ...
            print(
                f"Final Rewired train={round(metrics_mean[0], self.decimal)}+-{round(metrics_std[0], self.decimal)},"
    ...
    def single_fold(
        ...
        model = CoGNN(gumbel_args=gumbel_args, env_args=env_args, action_args=action_args, pool=self.pool).to(
            device=self.device
        )
        ...
        with tqdm.tqdm(total=self.max_epochs, file=sys.stdout) as pbar:
            best_losses_n_metrics, edge_ratios = self.train_and_test(
                dataset_by_split=dataset_by_split,
                model=model,
                optimizer=optimizer,
                scheduler=scheduler,
    ...
    def train_and_test(
        self, dataset_by_split: DatasetBySplit, model, optimizer, scheduler, pbar, num_fold: int
    ) -> Tuple[LossesAndMetrics, OptTensor]:
        ...
        for epoch in range(self.max_epochs):
            ...
            train_loss, train_metric, _ = self.test(
    ...
    def test(self, loader, model, split_mask_name: str, calc_edge_ratio: bool) -> Tuple[float, Any, Tensor]:
        model.eval()
        ...
        for data in loader:
            ...
            if calc_edge_ratio:
                edge_ratio_node_mask = self.dataset.get_edge_ratio_node_mask(
