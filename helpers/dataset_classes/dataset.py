...
class DataSet(Enum):
    ...
    def is_node_based(self) -> bool:
        return (
            self.get_family() in [DataSetFamily.heterophilic, DataSetFamily.homophilic]
    ...
    def scheduler(
        ...
        if self.get_family() is DataSetFamily.lrgb:
            ...
            return cosine_with_warmup_scheduler(
                optimizer=optimizer, num_warmup_epochs=num_warmup_epochs, max_epoch=max_epochs
            )
        elif self.get_family() in [DataSetFamily.social_networks, DataSetFamily.proteins]:
    ...
    def asserts(self, args):
        ...
        assert (
            self.get_family() in [DataSetFamily.social_networks, DataSetFamily.proteins] or args.fold is None
        ), "social networks and protein datasets are the only ones to use fold"
        assert (
            self.get_family() not in [DataSetFamily.social_networks, DataSetFamily.proteins] or args.fold is not None
